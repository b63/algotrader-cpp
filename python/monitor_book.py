...
def arbritrage_trade_watcher(target_orderbook: OrderBook, logger_obj: logging.Logger=tradelogger):
    def arbritrage_inner(orderbook: OrderBook):
        ...
        logger_obj.info(f"Found trade {orderbook.name} [{ask_quantity:.2E} @ ${min_ask})] -> [{target_orderbook.name} {bid_quantity:.2E} @ ${max_bid}], max profit: ${max_profit_text}")
...
async def monitor_market_feed(orderbook: OrderBook, exchangefeed : MarketFeed, max_messages=-1):
    ...
    async with websockets.connect(url, logger=socketlogger, ssl=True, max_size=MAX_MSG_SIZE, user_agent_header=None) as ws:
