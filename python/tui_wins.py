...
class TUIMarketFeedWin(TUIWin):
    ...
    def init_win(self):
        tuilogger.info(f"TUIMarketFeedWin<{self.header}> init_win before super(): _height {self._height},  _offset_y {self._offset_y}")
        super().init_win(curses.newwin(self._height, curses.COLS, self._offset_y, 0))
        assert self.win
        tuilogger.info(f"TUIMarketFeedWin<{self.header}> initialized window with {self.win.getmaxyx()}+({self._offset_y},0)")
    ...
    async def refresh_async(self):
        ...
        if self._prev_term_width != curses.COLS:
            ...
            summary_win = logcall(self.win.derwin, log=tuilogger)(self._summary_height, self._range_x, self._order_height+self._min_y, self._min_x)
...
class TUISummaryWin(TUIWin):
    ...
    def reprint(self):
        ...
        y = self._min_y
        self.addstr(y, self._min_x,   f"Max. Bid: {max_bid_price:6,.2f} ({self.orderbook.bid_quantity(max_bid_price):.2e})")
        self.addstr(y+1, self._min_x, f"Min. Ask: {min_ask_price:6,.2f} ({self.orderbook.ask_quantity(min_ask_price):.2e})")

        self.addstr(y+3, self._min_x, f"Min. Bid: {min_bid_price:6,.2f} ({self.orderbook.bid_quantity(min_bid_price):.2e})")
        self.addstr(y+4, self._min_x, f"Max. Ask: {max_ask_price:6,.2f} ({self.orderbook.ask_quantity(max_ask_price):.2e})")
    ...
    def _print_prices(self):
        ...
        for y in range(self._min_y, self._max_y+1):
            ...
            if  highlight_price_ask and range_start <= highlight_price_ask < range_end:
                self.chgat(y, x+ind_digit_front-1, self._orders_col_width + self._price_col_width - ind_digit_front, curses.A_REVERSE)
